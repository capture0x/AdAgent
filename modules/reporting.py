...
# ── AdAgent brand palette ─────────────────────────────────────────────────────
BRAND_PURPLE  = "#BE3CFF"   # electric purple (logo AD)
BRAND_MAGENTA = "#FF3CDC"   # hot magenta     (logo AGENT)
BRAND_DARK    = "#0d0d1a"   # near-black background
BRAND_DARK2   = "#13001f"   # slightly lighter dark
...
/* ── Header ── */
.report-header {
  background: linear-gradient(135deg, #0d0d1a 0%, #1a003a 50%, #2a0050 100%);
  color: #fff;
  padding: 48px 56px 36px;
  border-bottom: 4px solid #BE3CFF;
}
...
.section-title {
  ...
  border-bottom: 2px solid #BE3CFF;
...
def _html_stat_grid(counts: dict, owned_u: list, owned_m: list, cmds_run: int) -> str:
    ...
    cards.append(
        f'<div class="stat-card" style="border-top-color:#BE3CFF">'
        f'<div class="stat-num" style="color:#BE3CFF">{total}</div>'
...
<div class="report-header">
  <div style="display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:20px">
    <div>
      <div style="color:#BE3CFF;font-size:0.78em;font-weight:700;letter-spacing:2px;text-transform:uppercase;margin-bottom:8px">
      ...
      <div class="header-meta">
        ...
        <span><strong>Generated by:</strong> AdAgent v{VERSION}</span>
      </div>
    </div>
...
<div class="report-footer">
  <div style="margin-bottom:6px">
    Generated by <strong>AdAgent v{VERSION}</strong> &mdash; creator: tmrswrr &mdash;
    MITRE ATT&amp;CK® is a registered trademark of The MITRE Corporation.
  </div>
  <div style="color:#FF3CDC;font-weight:600">
...
<head>
  ...
  <title>{_html.escape(engagement)} — AdAgent Report</title>
  <style>{_CSS}</style>
</head>
<body>
{header}
...
def _build_navigator(engagement: str, findings: list[dict]) -> dict:
    ...
    return {
        "name": f"AdAgent — {engagement}",
        "versions": {"attack": "14", "navigator": "4.9.1", "layer": "4.5"},
        "domain": "enterprise-attack",
        "description": f"Generated by AdAgent v{VERSION} for engagement: {engagement}",
        "techniques": techniques_out,
        "gradient": {
            "colors": ["#ffffff", "#BE3CFF"],
            "minValue": 0,
            "maxValue": 1,
        },
        "legendItems": [
...
def _build_json(
    ...
    return {
        "schema_version":  "2.0",
        "generated_by":    f"AdAgent v{VERSION}",
...
def run():
    print_banner("ADAGENT REPORT", "HTML · Markdown · JSON · ATT&CK Navigator")

    try:
        engagement = SESSION.get("engagement") or prompt("Engagement name") or "AdAgent"
    except EOFError:
        engagement = SESSION.get("engagement") or "AdAgent"
    ...
    base = str(report_dir / f"adagent_report_{ts}")
